...
def VideoCLIP_XL_v2_v(videos: list[list[np.ndarray]]) -> list[np.ndarray]:
    print("model processing videos")
    processed_videos = []
    for vid in videos:
        vid_tube = []
        for fr in vid:
...
def VideoCLIP_XL_v2_t(texts: list[str]) -> list[np.ndarray]:
    print("loading model (t)")
    videoclip_xl = VideoCLIP_XL()
    state_dict = torch.load("models/videoclip_xl_v2/VideoCLIP-XL-v2.bin", map_location="cpu")
    videoclip_xl.load_state_dict(state_dict)
    videoclip_xl.eval()
